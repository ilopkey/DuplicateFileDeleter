...
#    list as the file to keep
    ...
    while True:
        ...
        for list_pos, list_item in enumerate(file_dup):
            
            if list_pos in lists_scanned:
                continue
                
            for file in list_item:
                file_del.append(file)

# Print out file_del and ask user for conformation
# to delete the files in file_del
				
for file_pos, file_item in enumerate(file_del):
	print '%i: %s' % (file_pos, file_item)

while True:
	delete_files = raw_input('Do you want to delete these files?'
							+'\n'
							+'Yes or No: ')
	...
	if delete_files == 'Yes':
		...
		print 'Valid Options: Yes or No. Case Sensitive'

# Delete all files in file_del
